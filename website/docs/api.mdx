...
### `disableDefaultStatusBarIOS` <span className="label ios">iOS</span> {/* #disableDefaultStatusBarIOS */}
...
Called when the backdrop or the back button is pressed. Receives a single `calledBy` argument telling which one, see [`DismissalSource`](#DismissalSource).
...
### `statusBar` <span className="label android">Android</span> <span className="label ios">iOS</span> {/* #statusBar */}
...
### `statusBarTranslucent` <span className="label android">Android</span> <span className="label basic deprecated">Deprecated</span> {/* #statusBarTranslucent */}
